...
func adminAPIKeyRequest(t *testing.T, body string) *http.Request {
	...
	ctx := context.WithValue(req.Context(), middleware.ContextKeyUserRole, "admin")
...
func TestUIIssuedLeastPrivilegeOrganizationKeyPassesOrganizationAPI(t *testing.T) {
	...
	if len(createResponse.Data.Scopes) != 1 ||
		createResponse.Data.Scopes[0] != authz.ScopeOrganizationsRead {
	...
	if storedScopes != authz.ScopeOrganizationsRead {
		t.Fatalf("stored scopes = %q, want %q", storedScopes, authz.ScopeOrganizationsRead)
	}
	...
	if !strings.Contains(schemaRec.Body.String(), authz.ScopeOrganizationsRead) ||
		strings.Contains(schemaRec.Body.String(), `"orgs:read"`) {
		t.Fatalf("OpenAPI does not use only the canonical organization scope")
	}
}
...
func TestAdminAPIKeyCreationRejectsDeprecatedAndUnknownScopes(t *testing.T) {
	...
	handler := NewAdminHandler(
		nil,
		services.NewAPIKeyServiceWithSecret(dbSvc.DB, "test-only-api-key-secret-at-least-32-bytes"),
		nil,
		nil,
	)

	for _, scope := range []string{"orgs:read", "organizations:reed"} {
...
func TestAdminPageEmitsCanonicalOrganizationScopes(t *testing.T) {
	...
	for _, scope := range []string{
		authz.ScopeOrganizationsRead,
		authz.ScopeOrganizationsWrite,
	} {
		if !strings.Contains(html, `value="`+scope+`"`) {
			t.Fatalf("admin page does not emit canonical scope %q", scope)
		}
	}
	if strings.Contains(html, `value="orgs:`) {
		t.Fatalf("admin page emits deprecated organization scope: %s", html)
	}
}
