...
func NewRouter(
	...
	r.Group(func(r chi.Router) {
		...
		r.With(RequireViewer).Get("/organizations", pageHandler.HandleEntitiesPage)
		r.With(RequireOperator).Get("/organizations/entities/new", pageHandler.HandleEntityForm)
		r.With(RequireOperator).Get("/organizations/entities/edit", pageHandler.HandleEntityForm)
		...
		r.With(RequireViewer).Get("/fleet", pageHandler.HandleFleetPage)
		r.With(RequireOperator).Get("/fleet/edit/{entity_id}", datastarHandler.HandleFleetEdit)
		r.With(RequireOperator).Get("/fleet/cancel/{entity_id}", datastarHandler.HandleFleetCancel)
		...
		r.Route("/api/organizations", func(r chi.Router) {
			r.With(RequireViewer).Get("/", datastarHandler.HandleListOrganizations)
		...
		r.Route("/api/entities", func(r chi.Router) {
			r.With(RequireViewer).Get("/", datastarHandler.HandleListEntities)
			r.With(RequireOperator).Post("/", datastarHandler.HandleCreateEntity)
			r.With(RequireOperator).Put("/{entity_id}", datastarHandler.HandleUpdateEntity)
			r.With(RequireOperator).Delete("/{entity_id}", datastarHandler.HandleDeleteEntity)
		})

		// Web API: Fleet (Datastar/SSE)
		r.Route("/api/fleet", func(r chi.Router) {
			r.With(RequireViewer).Get("/", datastarHandler.HandleListFleet)
			r.With(RequireOperator).Post("/", datastarHandler.HandleCreateFleetEntity)
			r.With(RequireOperator).Put("/update", datastarHandler.HandleUpdateFleetEntity)
			r.With(RequireOperator).Delete("/{entity_id}", datastarHandler.HandleDeleteFleetEntity)
			r.With(RequireViewer).Get("/sse", datastarHandler.HandleFleetSSE)
		})

		// Realtime SSE: Organizations
		r.With(RequireViewer).Get("/api/organizations/sse", datastarHandler.HandleOrganizationsSSE)
