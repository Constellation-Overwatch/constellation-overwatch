...
// mapping. Unknown values are retained so a migration cannot guess at a
...
func (s *Service) migrateAPIKeyScopeAliases() error {
	...
	for rows.Next() {
		...
		normalized, unknown := authz.MigrateStoredAPIKeyScopes(
			authz.ParseStoredAPIKeyScopes(record.current),
		)
		record.next = authz.FormatStoredAPIKeyScopes(normalized)
